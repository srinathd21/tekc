<?php
// edit-client.php (MODIFIED from add-client.php UI)
// Edit only these fields:
// client_name, mobile_number, email, client_type, company_name, office_address, site_address, state,
// password (optional),
// pan_number, gst_number, aadhaar_number (required only if Individual),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Password optional in edit, but if entered enforce min length
    ...
    /**
     * DUPLICATE CHECK (mobile/email) EXCLUDING CURRENT CLIENT
     */
    if (empty($validation_errors)) {
        $dup_sql = "SELECT id FROM clients WHERE (mobile_number = ? OR email = ?) AND id <> ? LIMIT 1";
        $dup_stmt = mysqli_prepare($conn, $dup_sql);
        if ($dup_stmt) {
            mysqli_stmt_bind_param($dup_stmt, "ssi", $mobile_number, $email, $id);
            mysqli_stmt_execute($dup_stmt);
            mysqli_stmt_store_result($dup_stmt);
            if (mysqli_stmt_num_rows($dup_stmt) > 0) {
                $validation_errors[] = "Another client already exists with same Mobile or Email";
            }
    ...
    if (empty($validation_errors)) {
        ...
        $final_password_hash = $client['password'];
        if ($password !== '') {
            $final_password_hash = password_hash($password, PASSWORD_DEFAULT);
        }
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                // redirect to manage-clients
                mysqli_stmt_close($stmt);
                header("Location: manage-clients.php?success=" . urlencode("Client updated successfully!"));
                exit;
            } else {
                $error = "Error updating client: " . mysqli_stmt_error($stmt);
            }
            mysqli_stmt_close($stmt);
        } else {
        ...
        <div id="contentScroll" class="content-scroll">
            <div class="container-fluid maxw">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="h3 fw-bold text-dark mb-1">Edit Client</h1>
                        <p class="text-muted mb-0">Update client and legal details</p>
                    </div>
                ...
                <form method="POST" id="clientForm" novalidate>
                    ...
                    <div class="form-panel">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <label for="password" class="form-label">New Password <span class="optional-badge">(Optional)</span></label>
                                ...
                                <div class="form-helper"><i class="bi bi-shield-lock"></i> If entered, must be 8+ characters</div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="office_address" name="office_address" rows="2"
                                          placeholder="Enter office address" required><?php echo e($_POST['office_address'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="site_address" name="site_address" rows="2"
                                          placeholder="Enter site address" required><?php echo e($_POST['site_address'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="pan_number" name="pan_number"
                                       value="<?php echo e($_POST['pan_number'] ?? ''); ?>"
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="gst_number" name="gst_number"
                                       value="<?php echo e($_POST['gst_number'] ?? ''); ?>"
                            ...
                            <div class="col-md-6" id="aadhaarWrap">
                                ...
                                <input type="text" class="form-control" id="aadhaar_number" name="aadhaar_number"
                                       value="<?php echo e($_POST['aadhaar_number'] ?? ''); ?>"
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="billing_address" name="billing_address" rows="2"
                                          placeholder="Enter billing address" required><?php echo e($_POST['billing_address'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="shipping_address" name="shipping_address" rows="2"
                                          placeholder="Enter shipping address" required><?php echo e($_POST['shipping_address'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="text-center mt-5">
                        <button type="submit" class="btn-submit">
                            <i class="bi bi-save"></i> Save Client Changes
                        </button>
                        <p class="text-muted mt-3" style="font-size: 14px;">
                            <i class="bi bi-info-circle me-1"></i>
                            Fields marked with * are mandatory. Password is optional (leave blank to keep existing).
                        </p>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Password optional in edit, but if entered validate length
        if (passwordInput && passwordInput.value && passwordInput.value.length > 0 && passwordInput.value.length < 8) {
