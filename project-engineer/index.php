...
// ---------------- NEW: Today Pending Tasks (ALL report types like today-tasks.php) ----------------
$reportTypes = [
...
if (!empty($siteIds)) {
  ...
  foreach ($reportTypes as $rt) {
    $types  = str_repeat('i', count($siteIds)) . "is"; // sites + employee + date
    $params = array_merge($siteIds, [$employeeId, $todayYmd]);
    ...
    if ($st) {
      ...
      while (mysqli_fetch_assoc($res)) {
        $completedAll++; // one report-type completed for one site
      }
      mysqli_stmt_close($st);
    }
  }
...
<head>
  ...
  <style>
    ...
    .stat-link:hover .stat-card{ border-color: rgba(45,156,219,.35); box-shadow: 0 16px 36px rgba(17,24,39,.08); transform: translateY(-1px); transition: .15s; }
...
<body>
      ...
      <div id="contentScroll" class="content-scroll">
        <div class="container-fluid maxw">
          ...
          <div class="row g-3 mb-3">
            ...
            <!-- NEW: Today Pending Tasks (like Alerts subtitle + clickable) -->
            <div class="col-12 col-md-6 col-xl-3">
              <a class="stat-link" href="today-tasks.php" title="Open Today Tasks">
                <div class="stat-card">
                  ...
                  <div>
                    ...
                    <div style="font-size:12px; color:#6b7280; font-weight:800;">
                      Pending reports from Today Tasks
                    </div>
                  </div>
                </div>
              </a>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      // Donut Chart: My completed vs pending today (DPR)
