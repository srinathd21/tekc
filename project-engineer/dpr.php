  ...
  // 5) NEW: If DPR already submitted for TODAY for the selected SITE by the logged-in EMPLOYEE,
  //         do NOT allow another submit on the same day. Next day allowed.
  //
  // NOTE: As per your request, I REMOVED the table creation and ALTER statements.
  //       (No schema changes here — you will handle from phpMyAdmin.)
  ...
  // NEW: check if already submitted for a date (employee+site+date)
  function hasDprForDate(mysqli $conn, int $employeeId, int $siteId, string $ymd): bool {
    if ($employeeId <= 0 || $siteId <= 0 || $ymd === '') return false;
    $st = mysqli_prepare($conn, "SELECT id FROM dpr_reports WHERE employee_id=? AND site_id=? AND dpr_date=? LIMIT 1");
    if (!$st) return false;
    mysqli_stmt_bind_param($st, "iis", $employeeId, $siteId, $ymd);
  ...
  function getEditableDprById(mysqli $conn, int $dprId, int $employeeId, string $designation): ?array {
    if ($dprId <= 0) return null;

    // Manager can edit DPR on managed sites only when editable_status=1
    if ($designation === 'manager') {
      ...
      if (!$st) return null;
      mysqli_stmt_bind_param($st, "ii", $dprId, $employeeId);
    } else {
      // Others can edit only own DPR and only when editable_status=1
      $sql = "
        SELECT *
        FROM dpr_reports
        WHERE id = ?
          AND employee_id = ?
          AND editable_status = 1
        LIMIT 1
      ";
      $st = mysqli_prepare($conn, $sql);
      if (!$st) return null;
  ...
  // ---------------- EDIT MODE (DPR update) ----------------
  $editId = isset($_GET['edit_id']) ? (int)$_GET['edit_id'] : 0;
  $editMode = false;
  $editRow = null;

  if ($editId > 0) {
    $editRow = getEditableDprById($conn, $editId, $employeeId, $designation);
    if ($editRow) {
      $editMode = true;
      // Lock to the site of the DPR being edited
      $siteId = (int)$editRow['site_id'];
    }
  }
  // ---------------- Default DPR No ----------------
  $todayYmd = date('Y-m-d');
  ...
  if ($siteId > 0) {
    ...
    // #01, #02 ... #09, #10 ...
    $defaultDprNo = '#' . str_pad($seq, 2, '0', STR_PAD_LEFT);
  }
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_dpr'])) {
    ...
    // Re-validate edit mode from DB on POST (important security check)
    if ($isUpdateRequest) {
      $editRow = getEditableDprById($conn, $postEditId, $employeeId, $designation);
      if (!$editRow) {
    ...
    // NEW: Block duplicate DPR on same day for same employee+site
    if ($error === '' && !$editMode && hasDprForDate($conn, $employeeId, $site_id, $dpr_date)) {
      $error = "You already submitted a DPR for this site on $dpr_date. You can submit again from the next day.";
    }
    ...
    $consRows = jsonCleanRows($consRows);
      ...
      // ---------------- UPDATE EXISTING DPR (Edit Mode) ----------------
      if ($editMode && $editId > 0) {

        // Optional: prevent changing site_id during edit (safety)
        if (!empty($editRow) && (int)$editRow['site_id'] !== $site_id) {
          $error = "Site mismatch. Cannot move DPR to another site.";
        }
        ...
        if ($error === '') {
          ...
          if (!$upd) {
            ...
            if (!mysqli_stmt_execute($upd)) {
              ...
              // Even if no rows changed (same data), lock it after manager-enabled edit flow is intended
              // MariaDB may return 0 affected rows for same values, so do a follow-up lock check only if needed
              if (mysqli_stmt_affected_rows($upd) < 1) {
                // Check if record still exists and editable (same values update scenario)
              ...
              $_SESSION['success'] = "DPR updated successfully.";
              header("Location: report.php?type=dpr");
              exit;
            }
            mysqli_stmt_close($upd);
          }
        ...
        // ---------------- INSERT NEW DPR ----------------
        $editableStatus = 2; // New DPR starts  (non-editable)
...
if (!$ins) {
  ...
  if ($editMode && $editRow) {
    $formSiteId   = (int)$editRow['site_id'];
    $formDprNo    = (string)($editRow['dpr_no'] ?? '');
    $formDprDate  = (string)($editRow['dpr_date'] ?? date('Y-m-d'));

    $defaultScheduleStart = (string)($editRow['schedule_start'] ?? '');
    $defaultScheduleEnd   = (string)($editRow['schedule_end'] ?? '');
  ...
  // NEW: check if already submitted today for selected site -> disable submit in UI only for create mode
  $alreadySubmittedToday = false;
  if (!$editMode && $formSiteId > 0) {
    $alreadySubmittedToday = hasDprForDate($conn, $employeeId, (int)$formSiteId, date('Y-m-d'));
  }
...
@media (max-width: 768px) {
      ...
      <div id="contentScroll" class="content-scroll">
        <div class="container-fluid maxw">
          ...
          <div class="title-row mb-3">
            <div>
              <h1 class="h-title"><?php echo $editMode ? 'Edit Daily Progress Report (DPR)' : 'Daily Progress Report (DPR)'; ?></h1>
              <p class="h-sub">
                <?php echo $editMode ? 'Update your existing DPR (manager-enabled edit)' : 'Submit your DPR for the selected project site'; ?>
              </p>
            </div>
          ...
          <?php if ($alreadySubmittedToday && $formSiteId > 0): ?>
            <div class="alert alert-warning" role="alert" style="border-radius:14px; font-weight:900;">
              <i class="bi bi-info-circle-fill me-2"></i>
              You already submitted DPR for this site today (<?php echo e(date('Y-m-d')); ?>). Submit will be available tomorrow.
            </div>
          <?php endif; ?>
            ...
            <div class="panel">
              ...
              <div class="grid-3">
                ...
                <div>
                  ...
                  <!-- IMPORTANT: keep DPR date as today only (optional). If you want allow selecting past dates, remove readonly. -->
  ...
  <select class="form-select" name="site_condition" required>
    ...
    <?php endforeach; ?>
            ...
            <div class="panel">
              <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
                ...
                <button type="button" class="btn btn-outline-primary btn-addrow" id="addWorkProgress">
                  <i class="bi bi-plus-circle"></i> Add Row
                </button>
              </div>
    ...
    <?php
      if (!empty($editWpRows)) {
        $wpRowsToRender = $editWpRows;
      } else {
        $wpRowsToRender = [[], [], [], []]; // default 4 blank rows for new DPR
      }

      foreach ($wpRowsToRender as $row):
        $wpStatus = (string)($row['status'] ?? '');
    ?>
      <tr class="wp-row">
        <td><input class="form-control" name="wp_task[]" value="<?php echo e($row['task'] ?? ''); ?>"></td>
        <td>
          <input class="form-control wp-duration" inputmode="numeric" name="wp_duration[]"
                placeholder="e.g. 3" value="<?php echo e($row['duration'] ?? ''); ?>">
        </td>
        <td><input type="date" class="form-control wp-start" name="wp_start[]" value="<?php echo e($row['start'] ?? ''); ?>"></td>
        <td><input type="date" class="form-control wp-end" name="wp_end[]" readonly value="<?php echo e($row['end'] ?? ''); ?>"></td>
        <td>
    ...
    <?php endforeach; ?>
            ...
            <div class="panel">
              ...
              <div class="d-flex justify-content-end mt-3">
                <button type="submit" class="btn-primary-tek" <?php echo ($formSiteId<=0 || $alreadySubmittedToday ? 'disabled' : ''); ?>>
                  <i class="bi <?php echo $editMode ? 'bi-pencil-square' : 'bi-check2-circle'; ?>"></i>
              ...
              <?php elseif ($alreadySubmittedToday): ?>
                <div class="small-muted mt-2"><i class="bi bi-info-circle"></i> DPR already submitted today. Submit will be enabled tomorrow.</div>
              <?php endif; ?>
            </div>
          </form>
  ...
  document.querySelectorAll('tr.wp-row').forEach(function(tr){
    ...
    // Auto-calc on page load for edit mode prefilled rows
    wpRecalcRow(tr);
  });
  ...
  function addRow(tbodyId, rowHtml){
    ...
    // If full <tr>...</tr> is passed
    if (html.toLowerCase().startsWith('<tr')) {
      tb.insertAdjacentHTML('beforeend', html);
      return;
    }

    // Otherwise assume only <td>...</td> cells are passed (existing sections)
    ...
    // Bind auto end-date calc for newly added row
    const tb = document.getElementById('workProgressBody');
    const tr = tb ? tb.lastElementChild : null;
    if (!tr) return;
