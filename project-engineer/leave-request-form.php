<?php
// leave-request-form.php — Leave Request (DPR-style UI)
// UI updated to match DPR page style/layout while keeping leave request functionality.
...
// ---------- FETCH EMPLOYEE'S MANAGER FROM reporting_manager FIELD ----------
$managerInfo = null;
...
// Fallback: try lookup by reporting_manager value directly
if (!$managerInfo) {
    $st = mysqli_prepare($conn, "SELECT reporting_manager FROM employees WHERE id = ?");
    if ($st) {
        mysqli_stmt_bind_param($st, "i", $employeeId);
        mysqli_stmt_execute($st);
        $res = mysqli_stmt_get_result($st);
        ...
        if ($emp && !empty($emp['reporting_manager'])) {
            ...
            ");
            if ($st) {
                mysqli_stmt_bind_param($st, "s", $emp['reporting_manager']);
                mysqli_stmt_execute($st);
                $res = mysqli_stmt_get_result($st);
                $managerInfo = mysqli_fetch_assoc($res);
                mysqli_stmt_close($st);
...
// Count leave requests (request-count based, as in your current logic)
$st = mysqli_prepare($conn, "
    SELECT
...
function handleAttachmentUpload($field_name) {
    ...
    if (!isset($_FILES[$field_name])) {
        return ['success' => false, 'error' => 'Attachment field missing'];
    }
    ...
    if (!in_array($file_ext, $allowed_extensions, true)) {
        return ['success' => false, 'error' => 'Allowed file types: PDF, DOC, DOCX, JPG, PNG, TXT'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Calculate leave days (display / summary)
    $leave_days = 0;
    ...
    // Check type-specific balance (request-count based)
    ...
    if (isset($upload_result['no_file']) && $upload_result['no_file']) {
        $attachment_path = null;
    ...
    // Manager values (nullable)
    $managerId   = $managerInfo['id'] ?? null;
    ...
    if (empty($validation_errors)) {
        ...
        if (!$stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                $_SESSION['flash_success'] = "Leave request submitted successfully! Request Number: " . $request_no;
                mysqli_stmt_close($stmt);
                header("Location: leave-request-form.php");
                exit;
            } else {
        ...
        // Cleanup uploaded file if validation failed after upload
        if (isset($upload_result['success']) && $upload_result['success'] && !empty($upload_result['path']) && file_exists($upload_result['path'])) {
    ...
    <div id="contentScroll" class="content-scroll">
      <div class="container-fluid maxw">
        ...
        <div class="title-row mb-3">
          <div>
            <h1 class="h-title">Leave Request</h1>
            <p class="h-sub">Submit your leave request for approval</p>
          </div>
          <div class="d-flex gap-2 flex-wrap">
            <span class="badge-pill"><i class="bi bi-person"></i> <?php echo e($employeeName); ?></span>
            <span class="badge-pill">
              <i class="bi bi-person-up"></i>
              <?php echo e($managerInfo['full_name'] ?? 'Manager Not Assigned'); ?>
            </span>
          </div>
        </div>
        ...
        <?php if (!empty($validation_errors)): ?>
          <div class="alert alert-danger" role="alert" style="border-radius:14px;">
            <div class="fw-bold mb-2"><i class="bi bi-exclamation-circle-fill me-2"></i>Please fix the following errors:</div>
            <ul class="mb-0">
        ...
        <div class="panel">
          <div class="sec-head">
            ...
            <div>
              <p class="sec-title mb-0">Employee & Reporting Manager</p>
              <p class="sec-sub mb-0">Auto-fetched from employee master</p>
            </div>
          </div>
        ...
        <div class="panel">
          <div class="sec-head">
            ...
            <div>
              ...
              <p class="sec-sub mb-0">Count-based balance (as per current system logic)</p>
            </div>
          </div>
          ...
          <div class="grid-4">
            <div class="balance-card">
              ...
              <div class="balance-note">Approved Used: <?php echo (int)$leaveBalance['approved']; ?></div>
            </div>
            <div class="balance-card">
              <div class="balance-type">Sick</div>
              <div class="balance-value"><?php echo (int)$leaveBalance['sick']; ?></div>
              <div class="balance-note">Quota</div>
            </div>
            <div class="balance-card">
            ...
            <div class="balance-card">
              ...
              <div class="balance-value" style="color:#16a34a;"><?php echo (int)$leaveBalance['available']; ?></div>
              <div class="balance-note">Pending: <?php echo (int)$leaveBalance['pending']; ?></div>
            </div>
          </div>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" id="leaveRequestForm" autocomplete="off">
          ...
          <div class="panel">
            <div class="sec-head">
              ...
              <div>
                ...
                <p class="sec-sub mb-0">Select leave type and date range</p>
              </div>
            </div>
            ...
            <div class="mt-3" id="leaveDaysWrap" style="display:none;">
              <div class="leave-days-info">
                <span><i class="bi bi-calendar-check me-2"></i><strong>Total Leave Days (Inclusive)</strong></span>
                <span class="leave-days-count" id="leaveDaysCount">0</span>
              </div>
            </div>
          </div>
          ...
          <div class="panel">
            ...
            <div class="grid-2 mt-2">
              ...
              <div>
                <label class="form-label">Manager (Read Only)</label>
                <input class="form-control" value="<?php echo e($managerInfo['full_name'] ?? 'Not Assigned'); ?>" readonly>
              </div>
            </div>
            ...
            <div class="mt-2">
              ...
              <textarea
                ...
                required
              ><?php echo e($_POST['request_description'] ?? ''); ?></textarea>
            </div>
            ...
            <div class="mt-2">
              ...
              <textarea
                ...
                name="handover_notes"
                rows="3"
                placeholder="Who will handle your work during your leave?"
              ><?php echo e($_POST['handover_notes'] ?? ''); ?></textarea>
            </div>
          </div>
          ...
          <div class="panel">
            ...
            <div class="file-upload-area" onclick="document.getElementById('attachments').click();">
              ...
              <small>PDF, DOC, DOCX, JPG, PNG, TXT (Max 5MB)</small>
              <input
                type="file"
                class="d-none"
                name="attachments"
            ...
            <div id="fileSelected" class="small-muted mt-2" style="display:none; color:#16a34a !important;">
          ...
          <div class="panel">
            <div class="sec-head">
              ...
              <div>
                <p class="sec-title mb-0">Submit Request</p>
                <p class="sec-sub mb-0">Your request will be sent to manager for approval</p>
              </div>
            </div>

            <div class="d-flex justify-content-end gap-2 flex-wrap">
              <a href="leave-request-list.php" class="btn-outline-tek">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const leaveDaysWrap = document.getElementById('leaveDaysWrap');
  ...
  function calcLeaveDays(){
    const fromVal = leaveFromDate ? leaveFromDate.value : '';
    const toVal = leaveToDate ? leaveToDate.value : '';

    if (!fromVal || !toVal) {
      if (leaveDaysWrap) leaveDaysWrap.style.display = 'none';
      return;
    }

    const from = parseYmd(fromVal);
    const to = parseYmd(toVal);
    if (!from || !to) {
      if (leaveDaysWrap) leaveDaysWrap.style.display = 'none';
      return;
    }
    ...
    leaveDaysCount.textContent = diff;
    leaveDaysWrap.style.display = 'block';
  }

  leaveFromDate?.addEventListener('change', function(){
    if (leaveToDate && leaveFromDate.value && leaveToDate.value && leaveToDate.value < leaveFromDate.value) {
      leaveToDate.value = leaveFromDate.value;
    }
    calcLeaveDays();
  });

  leaveToDate?.addEventListener('change', calcLeaveDays);
  ...
  form?.addEventListener('submit', function(e){
    ...
    const fromDate = leaveFromDate?.value || '';
    const toDate = leaveToDate?.value || '';
