...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dpr_edit_toggle'])) {
  if ($designation !== 'manager') {
    $_SESSION['success'] = 'Not allowed.';
    header("Location: report.php");
    exit;
  }

  $dprId = (int)($_POST['dpr_id'] ?? 0);
  $newStatus = (int)($_POST['editable_status'] ?? 2); // 1 or 2

  if ($dprId > 0 && in_array($newStatus, [1,2], true)) {
    $st = mysqli_prepare($conn, "
      UPDATE dpr_reports r
      INNER JOIN sites s ON s.id = r.site_id
      SET r.editable_status = ?
      WHERE r.id = ? AND s.manager_employee_id = ?
    ");
    if ($st) {
      mysqli_stmt_bind_param($st, "iii", $newStatus, $dprId, $employeeId);
      mysqli_stmt_execute($st);
      mysqli_stmt_close($st);

      $_SESSION['success'] = ($newStatus === 1) ? "DPR edit enabled." : "DPR edit locked.";
    } else {
      $_SESSION['success'] = "Failed to update DPR edit status.";
    }
  } else {
    $_SESSION['success'] = "Invalid request.";
  }

  // Keep filters after post
  $qs = $_GET;
  $url = 'report.php';
...
<head>
  ...
  .title-row{ align-items:stretch !important; }
  .title-row .d-flex.gap-2{ width:100%; flex-wrap:wrap; }
  ...
  .panel form .col-12.d-flex{
    justify-content: stretch !important;
    flex-wrap: wrap;
  }
  .panel form .col-12.d-flex .btn,
  .panel form .col-12.d-flex .btn-tek{
    flex: 1 1 140px;
...
@media (max-width: 767.98px){
  ...
  .h-title{ font-size:1.2rem !important; }
...
<td class="text-center">
  <?php
    $editStatus = (int)($r['editable_status'] ?? 2);

    // $canShowEdit = ($rtype === 'DPR' && $editStatus === 1);
    $canShowEdit = false;

    if ($rtype === 'DPR') {
      // list already respects role scope, so if DPR is editable=1 show edit button
      // (self users see only own; manager sees managed site reports)
      if ($editStatus === 1) {
        $canShowEdit = true;
      }
    }
  ...
  <?php if ($canShowEdit): ?>
    <a class="btn btn-sm btn-outline-warning"
       style="border-radius:10px; font-weight:900;"
       href="dpr.php?edit_id=<?php echo (int)$rid; ?>">
      <i class="bi bi-pencil-square"></i> Edit
    </a>
  <?php endif; ?>

  <?php if ($rtype === 'DPR' && $designation === 'manager'): ?>
    <form method="POST" action="report.php<?php echo !empty($_GET) ? ('?' . e(http_build_query($_GET))) : ''; ?>" style="display:inline;">
      <input type="hidden" name="dpr_edit_toggle" value="1">
      <input type="hidden" name="dpr_id" value="<?php echo (int)$rid; ?>">
      <input type="hidden" name="editable_status" value="<?php echo $editStatus === 1 ? 2 : 1; ?>">
      <button type="submit"
              class="btn btn-sm <?php echo $editStatus === 1 ? 'btn-outline-danger' : 'btn-outline-success'; ?>"
              style="border-radius:10px; font-weight:900;">
        <i class="bi <?php echo $editStatus === 1 ? 'bi-lock' : 'bi-unlock'; ?>"></i>
        <?php echo $editStatus === 1 ? 'Lock Edit' : 'Enable Edit'; ?>
      </button>
    </form>
  <?php endif; ?>
  ...
  <?php
    $basePrintFile = '';
    if ($rtype === 'DPR') {
      $basePrintFile = 'report-print.php';
    } elseif ($rtype === 'DAR') {
      $basePrintFile = 'report-dar-print.php';
    } elseif ($rtype === 'MPT') {
      $basePrintFile = 'report-mpt-print.php';
    } elseif ($rtype === 'MOM') {
      $basePrintFile = 'report-mom-print.php';
    } elseif ($rtype === 'MA') {
      $basePrintFile = 'report-ma-print.php';
    } else { // CHECKLIST
      $basePrintFile = 'report-checklist-print.php';
    }

    $viewPdfUrl = $basePrintFile . '?view=' . urlencode((string)$rid);
    $downloadPdfUrl = $basePrintFile . '?view=' . urlencode((string)$rid) . '&dl=1';
  ?>

  <!-- View / Print (opens PDF in new tab) -->
  <a class="btn btn-sm btn-outline-dark"
     style="border-radius:10px; font-weight:900;"
     href="<?php echo e($viewPdfUrl); ?>"
     target="_blank"
  ...
  <a class="btn btn-sm btn-outline-success"
     style="border-radius:10px; font-weight:900;"
     href="<?php echo e($downloadPdfUrl); ?>"
     rel="noopener noreferrer">
