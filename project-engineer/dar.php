...
// if ($siteId > 0) {
//   $seq = 1;
//   $st = mysqli_prepare($conn, "SELECT COUNT(*) AS cnt FROM dar_reports WHERE site_id=? AND dar_date=?");
//   if ($st) {
...
if ($siteId > 0) {
  ...
  $defaultDarNo = '#' . str_pad($seq, 2, '0', STR_PAD_LEFT);
  
  // Option 3: Just sequential number (if you want simple counting)
  // $defaultDarNo = '#' . ($siteId * 100 + $seq);
}
// ---------------- SUBMIT ----------------
$success = '';
$error = '';
