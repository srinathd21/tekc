<?php
// add-client.php (MODIFIED + mobile sidebar fix)
...
<head>
    ...
    .form-panel {
      ...
      box-shadow: var(--shadow, 0 8px 18px rgba(0,0,0,.04));
      padding: 25px;
      margin-bottom: 30px;
    }
    ...
    .section-subtitle { font-size: 14px; color: #718096; margin-top: 4px; margin-bottom: 0; }
    ...
    @media (max-width: 991.98px) {
      ...
      /* Main should not reserve sidebar width */
      .main{
        width: 100% !important;
    ...
    @media (max-width: 768px) {
      .content-scroll { padding: 14px 12px 12px; }
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-outline-secondary" type="button" id="generatePasswordBtn" aria-label="Generate password">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== Mobile Sidebar Overlay Fix (page-level fallback) =====
    ...
    function toggleSidebarMobile() {
        if (!sidebar || window.innerWidth > 991.98) return;
    ...
    function bindMenuButton() {
        ...
        // fallback: find hamburger icon inside buttons/links
        if (!btn) {
        ...
        if (btn && !btn.dataset.mobileSidebarBound) {
    ...
    function toggleAadhaarRequired() {
        ...
        if (aadhaarWrap) {
            const label = aadhaarWrap.querySelector('label');
            if (label) label.classList.toggle('required-label', isIndividual);
        }
    }
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Remove old inline invalid state
        form.querySelectorAll('.is-invalid').forEach(el => el.classList.remove('is-invalid'));

        // Remove previous dynamic validation alerts (avoid duplicates)
        document.querySelectorAll('.client-js-validation-alert').forEach(el => el.remove());
        ...
        if (email && email.value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) {
            valid = false;
            setInvalid(email);
            errorMessages.push('Invalid Mail ID format');
        }
        ...
        if (aadhaar && aadhaar.value && !/^[0-9]{12}$/.test(aadhaar.value)) {
            valid = false;
            setInvalid(aadhaar);
            errorMessages.push('Aadhaar must be 12 digits');
        }
        ...
        if (!valid) {
            ...
            alertDiv.className = 'alert alert-danger alert-dismissible fade show client-js-validation-alert';
