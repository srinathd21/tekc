<?php
// my-projects.php (Manager) — mobile sidebar fixed (overlay on mobile)
// - Shows ONLY projects assigned to logged-in Manager
...
<head>
  ...
  <style>
    ...
    .panel-menu{
      width:36px; height:36px; border-radius:12px; border:1px solid var(--border, #e5e7eb);
      background:#fff; display:grid; place-items:center; color:#6b7280;
    }
    ...
    .table td{
      vertical-align: top; border-color: var(--border, #e5e7eb);
    ...
    .btn-action:hover { background: var(--bg, #f8fafc); color: var(--blue, #2d9cdb); }
    ...
    .alert { border-radius: var(--radius, 16px); border:none; box-shadow: var(--shadow, 0 6px 20px rgba(0,0,0,.04)); margin-bottom: 20px; }
    ...
    @media (max-width: 991.98px) {
      ...
      /* Make sidebar off-canvas overlay on mobile */
      .sidebar{
      ...
      /* Works with different toggle class names */
      .sidebar.open,
      .sidebar.active,
      .sidebar.show,
      ...
      /* Main content should NOT reserve sidebar width on mobile */
      .main{
        width: 100% !important;
      ...
      /* Table wrapper should scroll instead of pushing page width */
      .panel{
      ...
      <div class="container-fluid maxw">
        ...
        <div class="panel mb-4">
          ...
          <div class="table-responsive">
            <table id="myProjectsTable" class="table align-middle mb-0 dt-responsive nowrap" style="width:100%">
              <thead>
                <tr>
                  <th>Project</th>
                  <th>Client</th>
...
<!-- Keep your existing sidebar script if it handles desktop behavior -->
...
<script>
  (function () {
    ...
    // Try multiple selectors because topbar button HTML may differ across pages
    function bindMenuButton() {
      ...
      // Fallback: find a button containing hamburger icon
      if (!btn) {
        const allBtns = document.querySelectorAll('button, a');
        for (const el of allBtns) {
          if (el.querySelector('.bi-list, .bi-list-ul, .bi-justify')) {
            btn = el;
            break;
          }
        }
      ...
      if (btn) {
        ...
        if (!btn.dataset.mobileSidebarBound) {
          btn.dataset.mobileSidebarBound = '1';
    ...
    function handleResize() {
      // When switching to desktop, clear mobile overlay state
      if (window.innerWidth > 991.98) {
        document.body.classList.remove('sidebar-open');
        const sidebar = getSidebarEl();
    ...
    $(function () {
      ...
      // Focus search after render (only desktop/tablet; on mobile this can open keyboard unexpectedly)
      setTimeout(function() {
        if (window.innerWidth > 991.98) {
